...
int main(int argc, char *argv[]) {
  ...
  // Calcolo il prezzo della Call e del Put per ogni simulazione, utilizzando un
  // campionamento diretto

  double z = 0;      // Variabile normale per il campionamento diretto
  double S = 0;      // Prezzo dell'asset al tempo T
  double Profit = 0; // Profitto al tempo T

  std::vector<double> Call(M); // Vettore che conterrà tutti i prezzi delle Call
                               // riportati al tempo t=0
  ...
  for (int i = 0; i < M; i++) { // Per ogni simulazione
    ...
    z = rnd.Gauss(0., T); // z è distribuito normalmente tra 0 e T
    S = S0 *
        exp((r - 0.5 * sigma * sigma) * T -
            sigma * z * sqrt(T)); // Formula per il prezzo dell'asset come GBM

    // Ora calcolo il profitto come max(0, S(T) - K), ovvero lo considero solo
    // se positivo
    Profit = S - K;
    if (Profit > 0) {
      Call[i] = Profit *
                exp(-r * T); // lo riporto al tempo t=0 tramite l'esponenziale
    } else {
      Put[i] =
          -Profit *
          exp(-r *
              T); // il profitto delle opzioni put è esattamente il contrario
    }
  }
  ...
  std::vector<double> time(time_divisions +
                           1); // vettore che conterrà t_0=0, t_1, ... , t_100

  for (int i = 0; i <= time_divisions; i++) {
    time[i] = i * T / time_divisions;
  }

  std::vector<double> Call2(M); // Vettore che conterrà tutti i prezzi delle
                                // Call riportati al tempo t=0
  ...
  for (int i = 0; i < M; i++) { // Per ogni simulazione
    ...
    for (int j = 1; j <= time_divisions; j++) {
      ...
      S *= exp(
          ...
          sigma * z *
              sqrt(time[j] -
                   time[j - 1])); // Formula per il prezzo dell'asset come
                                  // percorso di una GBM, ogni volta incremento
                                  // il tempo fino ad arrivare a t=T
    }
