...
int main(int argc, char *argv[]) {
  ...
  const double del_gs = 1.223; // range della probabilità di Transizione T tra due vettori posizione. Questo valore è scelto per ottenere un'accettazione di circa il 50% nel caso della pdf ground state. In coordinate cartesiante, T è uniforme in [x_n-del;x_n+del], [y_n-del;y_n+del] e [z_n-del;z_n+del]

  const double del_2p = 2.98; // analogo ma per il caso eccitato
...
// Caso Ground State
  for (int i = 0; i < M; i++) { // Per ogni simulazione
	  ...
	  double x_new = rnd.Rannyu(x-del_gs, x+del_gs); // posizione al passo successivo, usando la probabilità di Transizione
	  double y_new = rnd.Rannyu(y-del_gs, y+del_gs);
	  double z_new = rnd.Rannyu(z-del_gs, z+del_gs);
...
// dato che T è simmetrica, il rapporto q è dato solo dal rapporto tra la probabilità da campionare valutata in x_new e x_old
	  ...
	  if( r <= A ){ // se è vero accetto la nuova posizione, quindi x diveta x_new
		 ...
		 Accepted_gs[i] = 1.; // Conto il fatto che ho accettato
		  ...
		  Accepted_gs[i] = 0.;
	  } // Non accettato, la nuova posizione resta quella vecchia, quindi sovrascrivo x_new con un altro tentativo a partire sempre da x
...
// Caso eccitato
  ...
  for (int i = 0; i < M; i++) { // Per ogni simulazione
	  ...
	  double x_new = rnd.Rannyu(x-del_2p, x+del_2p); // posizione al passo successivo, usando la probabilità di Transizione
	  double y_new = rnd.Rannyu(y-del_2p, y+del_2p);
	  double z_new = rnd.Rannyu(z-del_2p, z+del_2p);
...
// dato che T è simmetrica, il rapporto q è dato solo dal rapporto tra la probabilità da campionare valutata in x_new e x_old
  ...
  Ave_Block(Accepted_gs, N, "dati_unif/Accettazioni_gs_unif.txt");
  Ave_Block(Position_gs, N, "dati_unif/Posizioni_gs_unif.txt");

  Ave_Block(Accepted_2p, N, "dati_unif/Accettazioni_2p_unif.txt");
  Ave_Block(Position_2p, N, "dati_unif/Posizioni_2p_unif.txt");
