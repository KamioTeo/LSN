...
// Implementazione del modello di Ising 1D
...
int main() { 
  ...
  if (Tsequence == 1) { // se voglio calcolare le grandezze nel range di temperatura
	...
	// for( int i=0; i< 100000; ++i)   // Equilibration, non serve
 //        Move(metro);
...
void Move(int metro) { // funzione che aggiorna il reticolo in base alla condizione di Metropolis o Gibbs
...
// Calcolo la probabilità di accettazione, a partire dalla differenza in energia tra la configurazione attuale e quella nuova in cui flippo lo spin: uso la funzione Boltzmann
      A = min(1., exp(-beta * (Boltzmann(-s[o],o) - Boltzmann(s[o],o)) )); // ovvero vale 1 se l'energia nuova è inferiore, quindi visto che dopo lo confronto con un numero random tra 0 e 1 viene sempre accettato
      if(rnd.Rannyu() < A) {
