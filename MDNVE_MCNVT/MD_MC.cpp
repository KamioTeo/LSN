...
int main() { 
  Input(); // inizializzo il sistema

int neq = 0; // numero step per equilibrare il sistema (in Verlet 2000 step per solido e liquido e 100k per gas, in Metropolis 200)
if (iNVET == 1) {
  neq = 200; 
} else {
...
//===========================================
// se voglio stampare i valori di temperatura ed energia istantanei DURANTE l'equilibrazione
  if (PrintEqValues == 1) {	  
	...
	for(int istep=1; istep <= neq; istep++) { // per ogni incremento temporale (step)
		...
		if (iNVET == 1) // Nell'algoritmo di Metropolis salvo l'andamento dell'energia potenziale per particella
			Energy[istep-1] = walker[iv]/(double)npart;
	}
	// salvo i valori su file esterno
	Print_File(Temperature, "./dati/Equilibration_T.dat");
	Print_File(Energy, "./Equilibration_U-N.dat");
...
// Ora inizio l'analisi: nella parte precedente ho trovato il numero di step, il numero di blocchi e il valore di delta ottimali per l'analisi. Qui equilibrio il sistema con quel numero di step
	...
	// se voglio stampare i valori di accettazione e fermare il programma
...
//===========================================================//
// Prima funzione, carico tutti i dati che mi servono
void Input(void) {
...
// Questa parte è uguale a quello che fa il costruttore che ho personalizzato dell'oggetto Random
...
// ATTENZIONE: densità, raggio di cutoff, temperatura dipendono dallo stato di aggregazione della sostanza, i valori riportati nei commenti sono quelli nell'input.in file
  ...
  ReadInput >> delta; // (qui 0.0005/0.2 liquido, 0.0005/0.12) è il dt^* d'integrazione, ovvero l'intervallino di tempo per integrare, in unità ridotte (dt^* = dt*sqrt(epsilon / (mass*sigma^2) )
...
// Ora che ho caricato tutti i parametri posso calcolare le correzioni di coda al potenziale LJ e alla pressione; le calcolo subito qui perché sono costanti
...
// tolgo la velocità di drift per particella alla rispettiva velocità, ovvero mi metto nel sdr del CM
    for (int i=0; i<npart; ++i) {
      ...
      vy[i] = vy[i] - sumv[1];
      vz[i] = vz[i] - sumv[2];
	// somma di tutti i moduli quadri delle singole velcoità, serve a calcolare l'energia cinetica media, che infatti dipende dalla somma delle singole velocità delle particelle al quadrato
...
// 0.5 * m * <v^*^2> * fs^2 = (3/2)*epsilon*T^* -> fs^2 = 3*epsilon*T^* / (m * <v^*^2>)
...
// uso l'algoritmo delle Periodic Boundary Condition per sfruttare la periodicità delle celle del reticolo
  ...
  for (int i=0; i<npart; ++i) {
    if(iNVET) { // se Monte Carlo NVT: N particelle, Volume e Temperatura costanti
// La posizione nuova proposta va accettata, quindi per ora setto la posizione iniziale come quella letta da file esterno
	  xold[i] = x[i];
      yold[i] = y[i];
      zold[i] = z[i];
    }
...
cout << endl << endl << "----------------------------" << endl << endl;
  ...
  return;
} // Fine definizione funzione void Input()
...
// Monte Carlo (NVT) move
  if(iNVET) {
    ...
    double fx[m_part], fy[m_part], fz[m_part]; // Array che contengono le forze nelle 3 direzioni subìte da tutte le particelle
...
// RECALL: r(t+dt) = 2*r(t) - r(t-dt) + dt^2 * a(t), ovvero mi serve la posizione a questo step, la posizione precedente e la forza totale su quella particelal
// xnew è r_x(t+dt), x[i] è r_x(t), xold è r_x(t-dt)
      xnew = Pbc( 2.0 * x[i] - xold[i] + fx[i] * pow(delta,2) );
      ynew = Pbc( 2.0 * y[i] - yold[i] + fy[i] * pow(delta,2) );
      znew = Pbc( 2.0 * z[i] - zold[i] + fz[i] * pow(delta,2) );

// sovrascrivo le velocità con quella nuova ma ricavata dall'algoritmo, ovvero calcolo v(t) grazie alla posizione a t+dt e alla posizione a t-dt
// al primo step coincide con v caricata da input file (infatti ho usato quella velocità per calcolare x a t-dt, ma qui sto riusando xold)
      vx[i] = Pbc(xnew - xold[i])/(2.0 * delta);
      vy[i] = Pbc(ynew - yold[i])/(2.0 * delta);
      vz[i] = Pbc(znew - zold[i])/(2.0 * delta);
// ora la posizione a t-dt diventa quella a t
      xold[i] = x[i];
      yold[i] = y[i];
      zold[i] = z[i];
// mentre la posizione a t diventa quella a t+dt
      x[i] = xnew;
      y[i] = ynew;
      z[i] = znew;
...
// controllo che sia dentro il raggio di cutoff, solo in questo caso ne conto il contributo
      if(dr < rcut) {
        vij = 1.0/pow(dr,12) - 1.0/pow(dr,6); // costributo ij-esimo al Potenziale di Lennard-Jones in unità ridotte (epsilon = sigma = 1) (manca un 4 moltiplicativo)
...
//update the histogram of g(r)
      idr = ig + int(dr/bin_size); // Questa è la disntanza nell'istogramma (quindi il numero di bin) alla quale si trova dr. Infatti, dr/bin_size è il numero di bin che ci vogliono per raggiungere una distanza dr, poi aggiungo l'indice ig=5 di partenza per traslare
...
*/
  ...
  walker[it] = (2.0 / 3.0) * kin/(double)npart; // Temperature, infatti E_k = (3/2) * N * K_b * T, scritto però in unità ridotte
...
if (iNVET) { // se è Metropolis devo aggiungere le tail corrections
  walker[iv] += vtail; // Potential energy, moltiplico il 4 che mancava al valore trovato prima per V_LJ
  walker[ip] += ptail;
}
...
// Visto che Averages è eseguito fuori dal ciclo degli step, blk_av qui è la somma di TUTTI i parametri ottenuti ad ogni step (a fine ciclo), quindi per trovare la media basta normalizzare sul numero di step per blocco (blk_norm) e per il numero di particelle (uso il fatto che la media temporale è circa la media spaziale sui microstati)
  ...
  // stima di g(r), è una funzione quindi devo calcolarla per ogni bin dell'istogramma
    ...
    for(int i = ig; i < n_props; i++){ // ciclo sui bin dell'istogramma
      ...
      rad[i-ig] = (i +1-ig)*bin_size; // la distanza nel bin k-esimo è data dal quel numero di bin moltiplicato per la larghezza dei bin. L'indice dell'istogramma non parte da 1 quindi devo traslarlo. Ad esempio il primo i=ig è ad una distanza r=bin_size
      r = rad[i-ig];
...
// Stampo su file i risultati di ogni blocco, i risultati cumulando i blocchi e l'errore cumulato
