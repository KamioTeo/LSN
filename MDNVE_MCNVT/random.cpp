...
// in ingresso gli do la lista delle simulazioni e il numero di blocchi in cui
// dividere
void Ave_Block(const vector<double> &sim_arr, int N, string File_name) {
  ...
  vector<double> ave(N); // lista che conterrà la media cumulativa delle simulazioni al variare
  ...
  // relativo errore di numeri pseudocasuali generati con distribuzione
  // uniforme. Questi sono divisi in N blocchi
  ofstream out;
  out.open(File_name);
  ...
  for (int i = 0; i < N; i++) { // per ogni blocco
    for (int j = 0; j <= i; j++) {
      sum_prog[i] += ave[j]; 	// SUM_{j=0,i} pi_j , sommo tutte le medie stimate
                             	// fino al blocco i+1-esimo
      su2_prog[i] += av2[j]; 	// SUM_{j=0,i} (pi_j)^2, sommo tutte le medie al
                             	// quadrato stimate fino al blocco i+1-esimo
    }
